...
try {
    ...
    Write-Host "❌ WSL não está funcionando. Execute o script anterior primeiro." -ForegroundColor Red
    Read-Host "Pressione Enter para sair"
    exit 1
}
...
if ($distributions -match "Ubuntu-22.04") {
    ...
    Write-Host ""
    Write-Host "💡 Para acessar o Ubuntu, use: wsl -d Ubuntu-22.04" -ForegroundColor Yellow
    Write-Host "💡 Para instalar o Buildozer, execute o script: instalar_buildozer_wsl.sh" -ForegroundColor Yellow
    Read-Host "Pressione Enter para sair"
    exit 0
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Ubuntu 22.04 LTS instalado com sucesso!" -ForegroundColor Green
    } else {
...
if ($testar -eq "S" -or $testar -eq "s" -or $testar -eq "Sim" -or $testar -eq "sim") {
    ...
    Write-Host "💡 Configure o usuário e senha quando solicitado" -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "💡 Para usar o Ubuntu mais tarde:" -ForegroundColor Yellow
